...
class GraphSearchTest(testClasses.TestCase):
    ...
    def getSolInfo(self, search):
        ...
        if solution is None:
            return None, problem.getExpandedStates(), None
        if type(solution) != type([]):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if solutionDict['solution'] is None or solutionDict['rev_solution'] is None:
            gold_solution = [solutionDict['solution'], solutionDict['rev_solution']]
        else:
...
class PacmanSearchTest(testClasses.TestCase):
    ...
    def getSolInfo(self, search, searchAgents):
        ...
        if solution is None:
            return None, problem._expanded, None
        if type(solution) != type([]):
            return None, None, 'The result of %s must be a list. (Instead, it is %s)' % (self.alg, type(solution))
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if solution is None:
            grades.addMessage('\tsolution length: empty')
        else:
